...
def calc_final_dist(_enc_batch_extend_vocab, vocab_dists, attn_dists, p_gens, batch_oov_len, vocab_size, batch_size):
    ...
    vocab_dists = [p_gen * dist for (p_gen, dist) in zip(p_gens, vocab_dists)]   #预测概率*pgen
    attn_dists = [(1-p_gen) * dist for (p_gen, dist) in zip(p_gens, attn_dists)]  #attention概率*（1-pgen），(dec_len,batch,enc_len)

    # Concatenate some zeros to each vocabulary dist, to hold the probabilities for in-article OOV words
    # the maximum (over the batch) size of the extended vocabulary
    extended_size = vocab_size + batch_oov_len   #填充后总长度
    extra_zeros = tf.zeros((batch_size, batch_oov_len))
    # list length max_dec_steps of shape (batch_size, extended_size)
    vocab_dists_extended = [tf.concat(axis=1, values=[dist, extra_zeros]) for dist in vocab_dists]  #向量扩充到后面，0填充
    ...
    indices = tf.stack((batch_nums, _enc_batch_extend_vocab), axis=2)  # shape (batch_size, enc_len, 2)
    shape = [batch_size, extended_size]
    # list length max_dec_steps (batch_size, extended_size) 插0使形状成shape
    attn_dists_projected = [tf.scatter_nd(indices, copy_dist, shape) for copy_dist in attn_dists]    #attention映射到概率部分
    ...
    return final_dists  #(dec_len,batch,vocab_size)