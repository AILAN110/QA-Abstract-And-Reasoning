...
class Vocab:
    ...
    def word_to_id(self, word):
        if word not in self.word2id:   #OOV
...
def example_generator(vocab, train_x_path, train_y_path, test_x_path, max_enc_len, max_dec_len, mode, batch_size,eval_x_path,eval_y_path):
    ...
    if mode == "train":
        ...
        for raw_record in train_dataset:
            ...
            enc_input_extend_vocab, article_oovs = article_to_ids(article_words, vocab)    #x序列做OOV扩展

            abstract_sentences = [""]
            abstract_words = abstract.split()
            abs_ids = [vocab.word_to_id(w) for w in abstract_words]
            abs_ids_extend_vocab = abstract_to_ids(abstract_words, vocab, article_oovs)   #y序列做OOV继续扩展
    ...
    if mode == "eval":
        eval_dataset_x = tf.data.TextLineDataset(eval_x_path)
        eval_dataset_y = tf.data.TextLineDataset(eval_y_path)
        test_dataset= tf.data.Dataset.zip((eval_dataset_x, eval_dataset_y))
        for raw_record in test_dataset:
            ...
            output = {
                ...
                "dec_len": 40,
                "article": article,
                "abstract": abstract,    #评估
                "abstract_sents": [],
                "sample_decoder_pad_mask": [],
                "sample_encoder_pad_mask": sample_encoder_pad_mask,
            }
            for _ in range(batch_size):
                yield output       #重复发beam_size次
...
def batch_generator(generator, vocab, train_x_path, train_y_path,
                    test_x_path, max_enc_len, max_dec_len, batch_size, mode,eval_x_path="",eval_y_path=""):
    dataset = tf.data.Dataset.from_generator(lambda: generator(vocab, train_x_path, train_y_path, test_x_path,
                                                               max_enc_len, max_dec_len, mode, batch_size,eval_x_path,eval_y_path),
                                             output_types={
                                                 "enc_len": tf.int32,
                                                 "enc_input": tf.int32,
...
def batcher(vocab, hpm):
    dataset = batch_generator(example_generator, vocab, hpm["train_seg_x_dir"], hpm["train_seg_y_dir"],
                              ...
                              hpm["max_dec_len"], hpm["batch_size"], hpm["mode"],eval_x_path=hpm["eval_seg_x_dir"],eval_y_path=hpm["eval_seg_y_dir"])
